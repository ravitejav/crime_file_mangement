...
class addmiss
{
  ...
  {
    if(isset($_POST['missname']) && isset($_POST['sid']) && isset($_POST['missage']) && isset($_POST['missaddress']) && isset($_POST['missplace']) && isset($_POST['missidno']) && isset($_POST['missphoneno']) && isset($_POST['regname']) && isset($_POST['regphoneno']) && isset($_POST['regadd']) && isset($_POST['addata']) && isset($_POST['device']) && isset($_POST['status']))
    {
      if(!empty($_POST['missname']) && !empty($_POST['sid']) && !empty($_POST['missage']) && !empty($_POST['missaddress']) && !empty($_POST['missplace']) && !empty($_POST['missidno'])&& !empty($_POST['missphoneno']) && !empty($_POST['regname']) && !empty($_POST['regphoneno']) && !empty($_POST['regadd']) && !empty($_POST['addata']) && !empty($_POST['device']) && !empty($_POST['status']))
      {
        $q="INSERT INTO `device` VALUES ('".$_POST['missname']."',".$_POST['sid'].",".$_POST['missage'].",'".$_POST['missaddress']."','".$_POST['missplace']."','".$_POST['missidno']."','".$_POST['missphoneno']."','','".$_POST['regname']."','".$_POST['regphoneno']."','".$_POST['regadd']."','".$_POST['addata']."','".$_POST['device']."','".$_POST['status']."')";
        $db=new database();
        $db->connect();
        $querye=new queryexc;
...
<html>
  ...
  <body>
    ...
    <form class="form-style-7" action="object.php" method="POST">
      <ul>
        <li>
        ...
        <li>
          <label for="missplace">where location</label>
          <input type="text" name="missplace" required="required" maxlength="250">
          <span>Where the device is lost</span>
        </li>
        <li>
        ...
        <li>
          <label for="device">What is lost</label>
          <input type="text" name="device" required="required" maxlength="60">
          <span>Which device is lost</span>
        </li>
        <li>
