...
class viewcrimi
{
  ...
  public function datae()
  {
    if (isset($_GET['criid']))
    {
      if(!empty($_GET['criid']))
      {
        $query="SELECT * FROM `criminal` WHERE `id`=".$_GET['criid'];
        $db=new database();
        $db->connect();
        $querye=new queryexc;
        ...
        {
          $this->j=0;
          echo "<script>alert('There is no criminal with id ".$_GET['criid']."  ')</script>";
        }
      }
    }
  }
...
<html>
  ...
  <body>
    ...
    <?php if($v->j==0){ ?>
    <form class="form-style-7" action="viewcrimi.php" method="GET">
      <ul>
        <li>
          <label for="criid">Criminal Id</label>
          <input type="text" name="criid" required="required" maxlength="10">
          <span>Enter the criminal id</span>
        </li>
        <li>
      ...
      <div style="overflow-x:auto">
        <center>
          <table class="view-table" border="3px">
            <tr>
              <td>Criminal Id</td><td><?php echo $v->data[1]['id'];?></td>
            </tr>
            <tr>
            ...
            <tr>
              <td>Crimes did by criminal</td><td><?php echo $v->data[1]['crimes']; ?></td>
            </tr>
            <tr>
