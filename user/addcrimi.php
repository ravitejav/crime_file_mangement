...
class addcr
{
  ...
  {
    if (isset($_POST['name']) && isset($_POST['sid']) && isset($_POST['age']) && isset($_POST['address']) && isset($_POST['idno']) &&  isset($_POST['idname']) &&  isset($_POST['phoneno']) &&  isset($_POST['crimes']) && isset($_POST['gen']) && isset($_POST['adddata']))
    {
      if(!empty($_POST['name']) && !empty($_POST['sid']) && !empty($_POST['age']) && !empty($_POST['address']) && !empty($_POST['idno']) &&  !empty($_POST['idname']) &&  !empty($_POST['phoneno']) &&  !empty($_POST['crimes']) && !empty($_POST['gen']) && !empty($_POST['adddata']))
      {
        $q="INSERT INTO `criminal` VALUES ('','".$_POST['name']."',".$_POST['sid'].",".$_POST['age'].",'".$_POST['address']."','".$_POST['idno']."','".$_POST['idname']."','".$_POST['phoneno']."','".$_POST['crimes']."','".$_POST['gen']."','".$_POST['adddata']."')";
        $db=new database();
        $db->connect();
        $querye=new queryexc;
...
<html>
  ...
  <body>
    ...
    <form class="form-style-7" action="addcrimi.php" method="POST">
      <ul>
        <li>
          <label for="name">Name</label>
          <input type="text" name="name" required="required" maxlength="80">
          <span>Enter the name of criminal</span>
        </li>
        <li>
        ...
        <li>
          ...
          <span>Enter the age of criminal</span>
        </li>
        <li>
        ...
        <li>
          ...
          <span>Enter the phone number of criminal</span>
        </li>
        <li>
          <label for="crimes">Crimes</label>
          <input type="text" name="crimes" required="required"  maxlength="250">
          <span>Mention the crimes did</span>
        </li>
        <li>
