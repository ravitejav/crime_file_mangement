...
class addmiss
{
  ...
  {
    if(isset($_POST['missname']) && isset($_POST['sid']) && isset($_POST['missage']) && isset($_POST['missaddress']) && isset($_POST['missplace']) && isset($_POST['missidno']) && isset($_POST['missidname']) && isset($_POST['lastseen']) && isset($_POST['missphoneno']) && isset($_POST['weapon']) && isset($_POST['regname']) && isset($_POST['regphoneno']) && isset($_POST['regadd']) && isset($_POST['addata']) && isset($_POST['status']))
    {
      if(!empty($_POST['missname']) && !empty($_POST['sid']) && !empty($_POST['missage']) && !empty($_POST['missaddress']) && !empty($_POST['missplace']) && !empty($_POST['missidno']) && !empty($_POST['missidname']) && !empty($_POST['lastseen']) && !empty($_POST['missphoneno']) && !empty($_POST['weapon']) && !empty($_POST['regname']) && !empty($_POST['regphoneno']) && !empty($_POST['regadd']) && !empty($_POST['addata']) && !empty($_POST['status']))
      {
        $q="INSERT INTO `murder` VALUES ('".$_POST['missname']."',".$_POST['sid'].",".$_POST['missage'].",'".$_POST['missaddress']."','".$_POST['missplace']."','".$_POST['missidno']."','".$_POST['missidname']."','".$_POST['lastseen']."','".$_POST['missphoneno']."','".$_POST['weapon']."','','".$_POST['regname']."','".$_POST['regphoneno']."','".$_POST['regadd']."','".$_POST['addata']."','".$_POST['status']."')";
        $db=new database();
        $db->connect();
        $querye=new queryexc;
...
<html>
  ...
  <body>
    ...
    <form class="form-style-7" action="murder.php" method="POST">
      <ul>
        <li>
          <label for="missname">Name</label>
          <input type="text" name="missname" required="required" maxlength="80">
          <span>Name of the person murdered</span>
        </li>
        <li>
